...
public class Loader {
    ...
    private <T> void showData(List<T> data, int totalPage, int totalCount) {
        ...
        if (STATE_NORMAL == state) {
            if (builder.onPageListener != null) {
                builder.onPageListener.load(data, totalPage, totalCount);
            }
        } else if (STATE_REFRESH == state) {
            builder.mRefreshLayout.finishRefresh();
            if (builder.onPageListener != null) {
                builder.onPageListener.refresh(data, totalPage, totalCount);
            }
        } else if (STATE_MORE == state) {
            builder.mRefreshLayout.finishRefreshLoadMore();
            if (builder.onPageListener != null) {
                builder.onPageListener.loadMore(data, totalPage, totalCount);
            }
        }
    }
    ...
    public static class Builder {
        ...
        public Loader build(Context context, Type type) {
            this.mType = type;
